...
## 就業保證
就業保證的重點就是給所有的公民一個宜居工資。 因為大部分的就業保證研究以美國為主（常聽到的每小時$15 USD），  因此我們需要先決定台灣的宜居工資有多少。 
...
5. 創造的工作數量  
    ...
    + **較保守的模型:** 就業保證只會僱用失業者，而不會讓灰心喪氣的工人重返勞動力  
    + **較實在的模型:** 將僱用失業者，並將沮喪的工人帶回勞動力 (假設有100％的灰心喪氣的工人會受到激勵加入就業保證)
...
除了增加就業人數，在台灣實施就業保證的話，至第六年還可以使台灣每個人的平均月工資提高16.6％，實質GDP增長差不多5％，並且把CPI只提高2.6%。
...
taiwan_jg_preds_vs_actuals %>%
  ...
  kable('html', align = c("l", "c", "c", "c", "c"),
        col.names = c("變數", "2018年的平均變化", "2018年的最大變化", 
                      "2018年的平均變化", "2018年的最大變化")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F, font_size = 12) %>%
