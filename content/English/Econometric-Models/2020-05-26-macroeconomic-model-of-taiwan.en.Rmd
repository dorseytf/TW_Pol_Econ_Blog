...
# Commodity Market
c_eqn   <- C ~ C + `YDD$` + PGDP + `STKVLE$` + CPI + Q1 + Q3
ibf_eqn <- IBF ~ IBF + IG + TDR1Y+ CPI + `STKVLE$` + `CAPSTOCK$` + SPIKE2008
exg_eqn <- EXG ~ PX + WPX + RX + GDPMAJOR + `ICSALES$` + STEP2008 + Q1 + Q2
exs_eqn <- EXS ~ EXS + `OVERSEAREV$` + VISITORS + RX + Q2 + Q4
img_eqn <- IMG ~ EXG + C + IPC + IBF + JJ
ims_eqn <- IMS ~ IMS + OUTBOUND
...
px_eqn <- PX ~ PX + RX + PM + WTVG
pd_eqn <- PD ~ PD + PM + GDP + SPIKE2008
pf_eqn <- PF ~ PF + FOOD + `WG$` + TYPH_T + Q1
po_eqn <- PO ~ PO + PM + PM + `WG$` + Q1 + Q2 + Q3
pc_eqn <- PC ~ PF + PO
pcg_eqn <- PCG ~ PO + WGP + PIG
pibf_eqn <- PIBF ~ PIBF + PCON + PCA + `WGST$` + SPIKE2008 + Q1 + Q2
pig_eqn <- PIG ~ PIG + PCON + `WGST$` + SPIKE2008
...
m1b_eqn <- M1B ~ CPI + GDP + `VSTOCK$` + TDR1Y + NOTERATE
tdr1y_eqn <- TDR1Y ~ REQ + IR + GDP + Q3
...
# Labor Market
ne_eqn <- NE ~ NE + GDP 
wg_eqn <- `WG$` ~ MINWGM + `GDP$` + `DEP$` + Q1

labor_eqn <- list(ne = ne_eqn, wg = wg_eqn)

# Tax Revenue
tax_eqn <- `TAX$` ~ `GDP$` + `EX$` + `C$` + `VSTOCK$` + Q1 + Q2 + Q3
...
jg_emp_df_realistic <- jg_emp_df %>%
  mutate(JG_REJOIN_LF = JOIN_LF * pct_rejoining_lf) %>%
  mutate(JJG = (JG_REJOIN_LF + NU)*PHASE_NU)

jg_dataset_realistic <- econ_data %>%
  mutate(PHASE_WG = case_when(year(quarterly_date) < jg_start_yr ~ 0,
                              year(quarterly_date) == jg_start_yr ~ phase_in_addtl,
                              year(quarterly_date) == jg_start_yr + 1 ~ 2*phase_in_addtl,
                              year(quarterly_date) == jg_start_yr + 2 ~ 3*phase_in_addtl,
                              ...
                              TRUE ~ 0),
         `(Intercept)` = 1) %>%
  left_join(jg_emp_df_realistic, by = "quarterly_date") %>%
  mutate(WJG     = MINWGM + PHASE_WG,
         YJG     = ((1000*JJG)*MINWGM*3)/1000000,
         CJG     = 0.15 * YJG,
         SPENDJG = YJG + CJG) %>%
...
long_jg_dataset_realistic <- jg_dataset_realistic %>%
  gather("metric", "value", -quarterly_date) %>%
  mutate(metric = str_replace_all(metric, "\\$", "_nom")) %>%
  mutate(metric = str_remove_all(metric, "`"))

jg_vars_used_realistic <- unique(long_jg_dataset_realistic$metric)

jg_data_list_realistic <- vector("list", length = length(jg_vars_used_realistic))
for(v in 1:length(jg_vars_used_realistic)){
  vals_to_ts <- long_jg_dataset_realistic$value[which(long_jg_dataset_realistic$metric == jg_vars_used_realistic[v])]
  start_yr <- min(year(long_jg_dataset_realistic$quarterly_date))
  start_qtr <- min(quarter(long_jg_dataset_realistic$quarterly_date[1]))
  
  jg_data_list_realistic[[v]] <- TIMESERIES(vals_to_ts, START = c(start_yr, start_qtr), FREQ = 4)
  names(jg_data_list_realistic)[v] <- jg_vars_used_realistic[v]
}


# Create TS to adjust NE by the number of ppl employed by JG and WG$ by the JG Wage phase in
real_constant_adj_list <- list(NE = TIMESERIES(jg_data_list_realistic$JJG['2012-01-01/2018-12-31'], 
                                               START = c(2012,1), 
                                               FREQ = 4),
                               WG_nom = TIMESERIES(jg_data_list_realistic$PHASE_WG['2012-01-01/2018-12-31'], 
                                                   START = c(2012,1), 
                                                   FREQ = 4))

# Adjust ECON_AFFAIRS and MINWGM to reflect JG spending and wage level
taiwan_bimets_model$modelData$ECON_AFFAIRS <- jg_data_list_realistic$ECON_AFFAIRS
...
taiwan_jg_model_realistic$simulation[[48]] <- NULL
...
In addition to increasing employment, implementing a JG in Taiwan would also increase the average monthly wages for everyone in Taiwan by nearly 18% by 2018Q4 (\$56,528 vs \$47,913), increase nominal GDP by nearly 1% in only a couple years, and have a neglible impact on CPI. 
...
taiwan_jg_preds_vs_actuals %>%
  ...
  mutate(data.ts = map(.x = data,
                       ...
                       start = 2012,
                       freq = 4)) %>%
  mutate(seas_adj = map(data.ts, .f = function(x) as.data.frame(seas(x)$data)$final)) %>%
  select(-c(data, data.ts)) %>%
  mutate(date = list(unique(taiwan_jg_preds_vs_actuals$quarterly_date[which(taiwan_jg_preds_vs_actuals$metric=="C")]))) %>%
  unnest(cols = c(seas_adj, date)) %>%
  spread(type, seas_adj) %>%
  mutate(pct_chg_cons = (`Conservative JG Model` - Actual)/Actual,
         pct_chg_real = (`Realistic JG Model` - Actual)/Actual) %>%
  group_by(metric) %>%
  summarize(`Conservative Model - Average Change for 2018` = paste0(round(mean(pct_chg_cons),3)*100, "%"),
            `Conservative Model - Max Change for 2018` = paste0(round(max(pct_chg_cons),3)*100, "%"),
            `Realistic Model - Average Change for 2018` = paste0(round(mean(pct_chg_real),3)*100, "%"),
            `Realistic Model - Max Change for 2018` = paste0(round(max(pct_chg_real), 3)*100, "%")) %>%
  kable('html', align = c("l", "c", "c", "c", "c"),
        col.names = c("Metric", "Average Change for 2018", "Max Change for 2018", 
                      "Average Change for 2018", "Max Change for 2018"),
        caption = "Simulated JG Impacts - Seasonally Adjusted") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F, font_size = 12) %>%
  add_header_above(c(" " = 1, "Conservative Model" = 2, "Realistic Model" = 2))
