...
Real GDP
$$
GDP = GDP_{-1} * (1 + \frac{\Delta{GDP}\%}{100})
$$
...
taiwan_bimets_model <- LOAD_MODEL_DATA(taiwan_bimets_model, econ_data_list)
...
bimet_formula_df <- bimet_formula_df %>%
...
for(i in 1:nrow(bimet_formula_df)){
  cat('\n')  
  cat(bimet_formula_df$model_name[i], ': \n') 
  cat(bimet_formula_df$formula[i], '\n')
  cat(bimet_formula_df$R_squared[i], ",", bimet_formula_df$Adj_R_squared[i])
  cat('\n')
}
...
While implementation strategies differ in the literature, we will use a simple 3 year phase-in for the new JG wage rate beginning in 2012. Because a JG is funded by the central government and administered at a local level, the funds for the JG will come from an increase to the Economic Affairs line item of Nominal Government Consumption. 

Assumptions:

1. JG Monthly Wage Rate = $28,000
2. Costs for administering the program are wages + materials and other costs (an additional 25% of wages)
3. The wage rate will be raised gradually over 3 years to the full $28,000/month
...
taiwan_jg_preds_vs_actuals %>%
  filter(metric %in% c("C", "GDP$", "GDP", "NE", "WG$")) %>%
  ggplot(aes(x = quarterly_date, y = value, color = type)) +
  geom_line() +
  facet_grid(metric ~ ., scales = "free") +
  scale_color_viridis_d(name = " ") +
